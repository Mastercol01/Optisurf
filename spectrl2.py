...
However, some minor changes where made to it. First, some of the comments and
descriptions were changed to better suit the standard being used in other 
modules of the Optisurf package. Second, the 'spectrl2' function was altered a
bit in order to accomodate the use of the shettel and Fenn models for the
...
def compute_direct_and_diffuse_normalized_spectra(sun_apzen, SP, rel_airmass, H2O, O3, AOD_500nm, alpha_500nm, 
    ...
    if not isinstance(ground_albedo, float) and not isinstance(ground_albedo, int):
...
if __name__ == '__main__':
    ...
    SP                       = Site_obj.site_data[(year, month, day)]["SP"]
    rel_airmass              = Site_obj.sun_data[(year, month, day)]["rel_airmass"]
    H2O                      = Site_obj.site_data[(year, month, day)]["H2O"]
    O3                       = Site_obj.site_data[(year, month, day)]["O3"]
    AOD_500nm                = Site_obj.site_data[(year, month, day)]["AOD_500nm"]
    alpha_500nm              = Site_obj.site_data[(year, month, day)]["alpha_500nm"]
    single_scattering_albedo = np.array(Site_obj.single_scattering_albedo[(year, month, day)].iloc[:,1:])
    spectrally_averaged_aaf  = Site_obj.site_data[(year, month, day)]["spectrally_averaged_aaf"]
