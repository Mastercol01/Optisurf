...
def fill_nans_using_laplace_1D(arr, num_iterations=500):
...
def fill_CDS_globe_nans_using_laplace(data, iterations = 20000):
    ...
    data : 2D numpy.array of floats
        ...
        circle of constant latitude equal to 90° (i.e, the geographic north 
        pole), data[-1,:] is the circle of constant latitude equal to -90° 
        (i.e, the geographic south pole), data[:,0] is the arc of constant 
    ...
    if lon_len != 2*lat_len:
        message = "WARNING: Length of axis 1 is not equal to 2 times the length"
        message = f"{message}  of axis 0. This function requires for the data to"
        message = f"{message}  be equally spaced and encompass the whole earth"
        message = f"{message}. That is only possible if data.shape[1] ="
        message = f"{message}  2*data.shape[0]"
        message = f"{message}. If these conditions are not satisfied, results"
        message = f"{message}. may be incorrect or misleading."
